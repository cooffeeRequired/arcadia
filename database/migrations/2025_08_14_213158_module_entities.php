...
class ModuleEntities extends AbstractMigration
{
    protected function migrate(): void
    {
        ...
        $table->addColumn('table_name', Types::STRING, ['length' => 255, 'notnull' => true]);
        ...
        $table->addColumn('table_exists', Types::BOOLEAN, ['default' => false]);
        ...
        $table->addIndex(['module_id'], 'idx_module_entities_module_id');
        $table->addIndex(['name'], 'idx_module_entities_name');
        $table->addIndex(['table_name'], 'idx_module_entities_table_name');
        $table->addIndex(['table_exists'], 'idx_module_entities_table_exists');

        // Foreign key na modules tabulku
        $table->addForeignKeyConstraint('modules', ['module_id'], ['id'], [
            'onDelete' => 'CASCADE',
            'onUpdate' => 'CASCADE'
        ], 'fk_module_entities_module_id');
    }
