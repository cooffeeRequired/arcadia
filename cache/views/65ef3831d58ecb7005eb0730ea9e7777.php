            ...
            <div class="border-t border-gray-200">
                <dl>
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><?php echo e($systemInfo['php_version'] ?? 'Neznámá'); ?></dd>
                    </div>
                    <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Doctrine ORM verze</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><?php echo e($systemInfo['doctrine_version'] ?? 'Neznámá'); ?></dd>
                    </div>
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Databázový driver</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><?php echo e($systemInfo['database_driver'] ?? 'Neznámý'); ?></dd>
                    </div>
                    <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Cache driver</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><?php echo e($systemInfo['cache_driver'] ?? 'Neznámý'); ?></dd>
                    </div>
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Prostředí</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><?php echo e($systemInfo['environment'] ?? 'Neznámé'); ?></dd>
                    </div>
                    <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Debug mód</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full <?php echo e(($systemInfo['debug_mode'] ?? false) ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'); ?>">
                                <?php echo e(($systemInfo['debug_mode'] ?? false) ? 'Zapnutý' : 'Vypnutý'); ?>
                    ...
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><?php echo e($systemInfo['last_backup'] ? $systemInfo['last_backup']->format('d.m.Y H:i') : 'Neznámá'); ?></dd>
                    </div>
                    <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Využití disku</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><?php echo e($systemInfo['disk_usage'] ?? 'Neznámé'); ?></dd>
                    </div>
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Využití paměti</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"><?php echo e($systemInfo['memory_usage'] ?? 'Neznámé'); ?></dd>
                    </div>
                    <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">OPcache</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full <?php echo e(($systemInfo['opcache_enabled'] ?? false) ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'); ?>">
                                <?php echo e(($systemInfo['opcache_enabled'] ?? false) ? 'Aktivní' : 'Neaktivní'); ?>

                            </span>
                            <?php if($systemInfo['opcache_enabled'] && $systemInfo['opcache_status']): ?>
                                <div class="mt-2 text-xs text-gray-500">
                                    <div>Paměť: <?php echo e(number_format($systemInfo['opcache_status']['memory_usage']['used_memory'] / 1024 / 1024, 2)); ?> MB / <?php echo e(number_format($systemInfo['opcache_status']['memory_usage']['free_memory'] / 1024 / 1024, 2)); ?> MB</div>
                                    <div>Hit rate: <?php echo e(number_format($systemInfo['opcache_status']['opcache_statistics']['opcache_hit_rate'], 2)); ?>%</div>
                                    <div>Kompilované soubory: <?php echo e($systemInfo['opcache_status']['opcache_statistics']['num_cached_scripts']); ?></div>
                                </div>
                            <?php endif; ?>
                        </dd>
                    </div>
        ...
        <div class="bg-white shadow sm:rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                ...
                <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <?php if($systemInfo['opcache_enabled']): ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /custom/repositories/private/Arcadia/resources/views/settings/system.blade.php ENDPATH**/ ?>