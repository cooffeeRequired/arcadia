...
@section('title', 'Upravit workflow')
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Upravit workflow</h1>
            <p class="text-gray-600 mt-2">Upravte nastavení automatizace pro vaše business procesy</p>
        </div>
    ...
    <form method="POST" action="/workflows/{{ $workflow->getId() }}" class="space-y-8">
        @method('PUT')
        <!-- Základní informace -->
        <div class="card">
            ...
            <div class="p-6 space-y-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ $old['name'] ?? $workflow->getName() }}"
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              ...
                              placeholder="Popis účelu workflow">{{ $old['description'] ?? $workflow->getDescription() }}</textarea>
                ...
                <div class="flex items-center">
                    <input type="checkbox" id="is_active" name="is_active" value="1"
                           {{ ($old['is_active'] ?? $workflow->isActive()) ? 'checked' : '' }} class="form-checkbox">
                ...
                <div>
                    ...
                    <select id="priority" name="priority" class="form-input">
                        <option value="0" {{ ($old['priority'] ?? $workflow->getPriority()) == 0 ? 'selected' : '' }}>Nízká (0)</option>
                        <option value="5" {{ ($old['priority'] ?? $workflow->getPriority()) == 5 ? 'selected' : '' }}>Střední (5)</option>
                        <option value="10" {{ ($old['priority'] ?? $workflow->getPriority()) == 10 ? 'selected' : '' }}>Vysoká (10)</option>
                    </select>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                <div>
                    ...
                    <select id="trigger_type" name="trigger_type" class="form-input @if(isset($errors['trigger_type'])) border-red-500 @endif">
                        ...
                        <option value="{{ $value }}" {{ ($old['trigger_type'] ?? $workflow->getTriggerType()) == $value ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="mt-4">
                    ...
                    <textarea id="trigger_config" name="trigger_config" rows="4"
                              ...
                              placeholder='{"days_overdue": 30}'>{{ $old['trigger_config'] ?? json_encode($workflow->getTriggerConfig()) }}</textarea>
        ...
        <div class="card">
            ...
            <div class="p-6">
                <div id="conditions-container">
                    @php
                        $conditions = $old['conditions'] ?? $workflow->getConditions();
                        ...
                        <div class="condition-item border rounded-lg p-4 mb-4">
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <div>
                                    <label class="form-label">Typ podmínky</label>
                                    <select class="condition-type form-input">
                                        <option value="field_exists" {{ $condition['type'] ?? '' == 'field_exists' ? 'selected' : '' }}>Pole existuje</option>
                                        <option value="field_value" {{ $condition['type'] ?? '' == 'field_value' ? 'selected' : '' }}>Hodnota pole</option>
                                        <option value="field_empty" {{ $condition['type'] ?? '' == 'field_empty' ? 'selected' : '' }}>Pole je prázdné</option>
                                        <option value="field_not_empty" {{ $condition['type'] ?? '' == 'field_not_empty' ? 'selected' : '' }}>Pole není prázdné</option>
                                    </select>
                                </div>
                                <div>
                                    <label class="form-label">Pole</label>
                                    <input type="text" class="condition-field form-input" placeholder="email, status, value" value="{{ $condition['field'] ?? '' }}">
                                </div>
                                <div class="operator-container">
                                    <label class="form-label">Operátor</label>
                                    <select class="condition-operator form-input">
                                        <option value="equals" {{ ($condition['operator'] ?? '') == 'equals' ? 'selected' : '' }}>Rovná se</option>
                                        <option value="not_equals" {{ ($condition['operator'] ?? '') == 'not_equals' ? 'selected' : '' }}>Nerovná se</option>
                                        <option value="contains" {{ ($condition['operator'] ?? '') == 'contains' ? 'selected' : '' }}>Obsahuje</option>
                                        <option value="greater_than" {{ ($condition['operator'] ?? '') == 'greater_than' ? 'selected' : '' }}>Větší než</option>
                                        <option value="less_than" {{ ($condition['operator'] ?? '') == 'less_than' ? 'selected' : '' }}>Menší než</option>
                                    </select>
                                </div>
                                <div class="value-container">
                                    <label class="form-label">Hodnota</label>
                                    <input type="text" class="condition-value form-input" placeholder="Hodnota pro porovnání" value="{{ $condition['value'] ?? '' }}">
                                </div>
                            </div>
                            <button type="button" class="remove-condition mt-2 text-red-600 hover:text-red-800 text-sm">
                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <input type="hidden" name="conditions" id="conditions-input" value="{{ $old['conditions'] ?? json_encode($workflow->getConditions()) }}">
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="p-6">
                <div id="actions-container">
                    @php
                        $actions = $old['actions'] ?? $workflow->getActions();
                    ...
                    @else
                        ...
                        <div class="action-item border rounded-lg p-4 mb-4">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    <label class="form-label">Parametr 1</label>
                                    <input type="text" class="action-param1 form-input" placeholder="Název, e-mail, template" value="{{ $action['param1'] ?? '' }}">
                                </div>
                                <div>
                                    <label class="form-label">Parametr 2</label>
                                    <input type="text" class="action-param2 form-input" placeholder="Hodnota, předmět, popis" value="{{ $action['param2'] ?? '' }}">
                                </div>
                            </div>
                            <button type="button" class="remove-action mt-2 text-red-600 hover:text-red-800 text-sm">
                                <svg class="w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <input type="hidden" name="actions" id="actions-input" value="{{ $old['actions'] ?? json_encode($workflow->getActions()) }}">
            </div>
        </div>
