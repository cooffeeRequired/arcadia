                ...
                <form method="POST" action="/settings/profile" class="mt-6 space-y-6">
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text" name="role" id="role" value="{{ $user['role'] ?? '' }}" readonly class="mt-1 bg-gray-50 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>

                        <div>
                            <label for="last_login" class="block text-sm font-medium text-gray-700">Poslední přihlášení</label>
                            <input type="text" name="last_login" id="last_login" value="{{ $user['last_login'] ? $user['last_login']->format('d.m.Y H:i') : '' }}" readonly class="mt-1 bg-gray-50 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>

                        <div>
                            <label for="created_at" class="block text-sm font-medium text-gray-700">Účet vytvořen</label>
                            <input type="text" name="created_at" id="created_at" value="{{ $user['created_at'] ? $user['created_at']->format('d.m.Y') : '' }}" readonly class="mt-1 bg-gray-50 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>
                    </div>
