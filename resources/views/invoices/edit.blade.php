...
<div class="px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <a href="/invoices/{{ $invoice->getId() }}" class="text-blue-600 hover:text-blue-900 text-sm font-medium">
            ← Zpět na fakturu
        </a>
    </div>
        ...
        <div class="px-4 py-5 sm:p-6">
            ...
            <form method="POST" action="/invoices/{{ $invoice->getId() }}" class="mt-6 space-y-6">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <select id="customer_id" name="customer_id" required class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                            ...
                            @if(isset($customers) && count($customers) > 0)
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->getId() }}" {{ $invoice->getCustomer()->getId() == $customer->getId() ? 'selected' : '' }}>
                                        {{ $customer->getName() }}
                                    </option>
                                @endforeach
                            @endif
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                            <option value="draft" {{ $invoice->getStatus() == 'draft' ? 'selected' : '' }}>Koncept</option>
                            <option value="sent" {{ $invoice->getStatus() == 'sent' ? 'selected' : '' }}>Odeslána</option>
                            <option value="paid" {{ $invoice->getStatus() == 'paid' ? 'selected' : '' }}>Zaplacena</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date" id="due_date" value="{{ $invoice->getDueDate()->format('Y-m-d') }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>

                    <div>
                        <label for="tax_rate" class="block text-sm font-medium text-gray-700">DPH (%)</label>
                        <input type="number" name="tax_rate" id="tax_rate" value="{{ $invoice->getTaxRate() }}" min="0" max="100" step="0.1" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
                    ...
                    <div>
                        ...
                        <select id="currency" name="currency" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                            <option value="CZK" {{ $invoice->getCurrency() == 'CZK' ? 'selected' : '' }}>CZK</option>
                            <option value="EUR" {{ $invoice->getCurrency() == 'EUR' ? 'selected' : '' }}>EUR</option>
                            <option value="USD" {{ $invoice->getCurrency() == 'USD' ? 'selected' : '' }}>USD</option>
                        </select>
                    </div>
                </div>
                        ...
                        <div class="invoice-item border border-gray-200 rounded-lg p-4 mb-4">
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-5">
                                <div class="sm:col-span-2">
                                    ...
                                    <input type="text" name="items[{{ $index }}][description]" value="{{ $item->getDescription() }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Množství</label>
                                    <input type="number" name="items[{{ $index }}][quantity]" value="{{ $item->getQuantity() }}" min="0" step="0.01" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Jednotka</label>
                                    <input type="text" name="items[{{ $index }}][unit]" value="{{ $item->getUnit() }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Cena za jednotku</label>
                                    <input type="number" name="items[{{ $index }}][unit_price]" value="{{ $item->getUnitPrice() }}" min="0" step="0.01" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                </div>
                            </div>
                            @if($index > 0)
                            <div class="mt-2">
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ $invoice->getNotes() }}</textarea>
                </div>

                <div class="flex justify-end space-x-3">
                    <a href="/invoices/{{ $invoice->getId() }}" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        Zrušit
                    </a>
...
// Přidání event listenerů pro existující tlačítka odstranění
